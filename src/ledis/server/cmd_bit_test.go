...
func testBitGetSet(t *testing.T) {
	...
	//	get / set
	if v, err := ledis_client.Int(c.Do("bgetbit", key, 1024)); err != nil {
		t.Fatal(err)
	} else if v != 0 {
		t.Fatal(v)
	}

	if ori, err := ledis_client.Int(c.Do("bsetbit", key, 1024, 1)); err != nil {
		t.Fatal(err)
	} else if ori != 0 {
		t.Fatal(ori)
	}
	...
	if v, err := ledis_client.Int(c.Do("bgetbit", key, -1)); err != nil {
		t.Fatal(err)
	} else if v != 1 {
		t.Fatal(v)
	}

	if v, err := ledis_client.Int(c.Do("bgetbit", key, -25)); err != nil {
		t.Fatal(err)
	} else if v != 1 {
		t.Fatal(v)
	}
	...
	//	delete
	if drop, err := ledis_client.Int(c.Do("bdelete", key)); err != nil {
		t.Fatal(err)
	} else if drop != 1 {
		t.Fatal(drop)
	}
...
func testBitMset(t *testing.T) {
	...
	for _, pos := range fillPos {
		v, err := ledis_client.Int(c.Do("bgetbit", key, pos))
		if err != nil || v != 1 {
			t.Fatal(pos)
		}
	}
...
func testBitCount(t *testing.T) {
	...
	if n, err := ledis_client.Int(c.Do("bcount", key)); err != nil {
		t.Fatal(err)
	} else if n != sum {
		t.Fatal(n)
	}
...
func testBitOpt(t *testing.T) {
	...
	if v, _ := ledis_client.String(c.Do("bget", dstk)); v != "" {
		t.Fatal(v)
	}
	...
	if v, _ := ledis_client.Int(c.Do("bgetbit", dstk, 100)); v != 1 {
		t.Fatal(v)
	}

	if v, _ := ledis_client.Int(c.Do("bgetbit", dstk, 20)); v != 0 {
		t.Fatal(v)
	}

	if v, _ := ledis_client.Int(c.Do("bgetbit", dstk, 40)); v != 0 {
		t.Fatal(v)
	}
	...
	if v, _ := ledis_client.Int(c.Do("bgetbit", dstk, 20)); v != 1 {
		t.Fatal(v)
	}

	if v, _ := ledis_client.Int(c.Do("bgetbit", dstk, 40)); v != 1 {
		t.Fatal(v)
	}
	...
	if v, _ := ledis_client.Int(c.Do("bgetbit", dstk, 100)); v != 0 {
		t.Fatal(v)
	}
